...
const SignUp = () => {
  ...
  return (
    ...
    <Box>
        <Text color='blue.500' fontSize={50} textAlign='center' fontFamily='sans-serif'>SignUp</Text>
        <FormControl>
            <Input variant='filled' placeholder='Display Name' my={5} 
            onChange={(e) => {
      ...
      <InputRightElement width='4.5rem'>
        <Button h='1.75rem' size='sm' onClick={handleClick}>
          {show ? 'Hide' : 'Show'}
            ...
            <Input variant='filled' placeholder='Confirm Password' my={5} 
              type={show ? 'text' : 'password'}
              onChange={(e) => {
                setConfirmPassword(e.target.value)
            }}
            />
            <Button backgroundColor='blue.500' color='white' onClick={handleForm}>{loading ? 
